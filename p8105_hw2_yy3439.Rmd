...
The `Mr Trash Wheel` sheet has `r nrow(mr_trash_wheel_data)` observations. The `Professor Trash Wheel` sheet has `r nrow(professor_tw_data)` observations, and the `Gwynna Trash Wheel` sheet has `r nrow(gwynnda_tw_data)` observations. Specifically, all these data is calculated after we omit rows with no specific dumpster data. 
In addition, the combined dataset `trash_wheel` contains `r nrow(trash_wheel)` observations and `r ncol(trash_wheel)` columns. 
